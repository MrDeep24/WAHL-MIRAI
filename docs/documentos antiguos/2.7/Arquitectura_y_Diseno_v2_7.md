...
## Sistema de Votaciones Digitales (Wahl Mirai) — Versión 2.7

Este documento describe la arquitectura técnica, la estructura del proyecto en ASP.NET Core MVC / Web API + SPA, el modelo de datos relacional completo (13 tablas) y los flujos de control del sistema de elecciones **Wahl Mirai**.
...
└── wwwroot/                       # SPA Única Client-Side
    ...
    └── js/
        ...
        │   ├── ayuda.js            # Acordeón FAQ + banner de primera visita (M08)
        │   └── pqr.js              # Formulario de creación y panel de gestión de PQR (M08)
        └── services/              # Cliente HTTP REST (Fetch + JWT Headers) y WebSocket Client
```

---

## 3. Modelo de Datos (Diagrama Entidad-Relación — 13 Tablas)

El modelo de base de datos de **Wahl Mirai v2.7** contempla 13 tablas organizadas relacionalmente, garantizando el anonimato del voto (RN-3), el censo persistente (RN-6), la auditoría (RN-8), el envío progresivo de correos (RN-9), el soft-delete de procesos (RN-7.1) y el canal de Ayuda/PQR del elector (M08):
...
erDiagram
    ...
    pqr_tickets {
        ...
        TEXT admin_response
        INT responded_by_voter_id FK "Nullable, Admin que resuelve"
...
### 5.8 M08 — Ayuda, Tutorial y PQR
*   **RF-M08-00 (Sección de Ayuda Estática Ilustrada):** Panel tipo acordeón con un tema por cada flujo relevante (login, recuperación, votación, perfil, resultados), cada uno con una ilustración SVG estática de pasos (`wwwroot/img/ayuda/`) y un enlace directo a 'Crear PQR' si la duda persiste. Contenido embebido en `ayuda.js`, sin persistencia en base de datos. Incluye un banner de una sola aparición en el primer inicio de sesión, controlado exclusivamente por `localStorage` del navegador (sin impacto en el esquema ni en `voters`).
*   **RF-M08-01 (Creación de PQR):** El elector autenticado radica una PQR (asunto + mensaje en texto libre) desde `pqr.js`. Se inserta un registro en `pqr_tickets` con `status = 'ABIERTO'`. En la sección de Ayuda (`Views/Pqr/Index.cshtml`), un nuevo endpoint `GET /Pqr/Mine` (rol `ELECTOR`, filtrado por su propio `voter_id` desde los claims de sesión) alimenta el listado de su historial de PQR previas con estado y `admin_response` si ya fue resuelta. Como Ayuda es visible para cualquier rol autenticado, este bloque se renderiza y consulta condicionalmente solo cuando `User.IsInRole("ELECTOR")`.
*   **RF-M08-02 (Gestión y Resolución de PQR):** El Administrador lista y filtra tickets por estado desde `PqrController.cs`, y registra una única `admin_response` que cambia el `status` a `'RESUELTO'`, encolando la notificación en `email_queue` (`email_type = 'RESPUESTA_PQR'`). **Decisión de diseño:** a diferencia de los demás módulos administrativos, esta operación no pasa por `AuditService.cs` ni genera entradas en `audit_log`; su trazabilidad queda cubierta por los propios campos de `pqr_tickets` (`status`, `responded_by_voter_id`, `responded_at`).

### 5.9 Servicio de Cola de Envío Progresivo (RN-9)
