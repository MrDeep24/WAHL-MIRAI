...
*   **RF-M08-01 (Creación de PQR):** El elector autenticado radica una PQR (asunto + mensaje en texto libre) desde `pqr.js`. Se inserta un registro en `pqr_tickets` con `status = 'ABIERTO'`. Debajo del formulario, un nuevo endpoint `GET /Pqr/Mine` (rol `ELECTOR`, filtrado por su propio `voter_id` desde los claims de sesión) alimenta el listado de su historial de PQR previas con estado y `admin_response` si ya fue resuelta.
