...
| **Flujo normal** | 1. El Administrador accede a 'Gestión de Censo'.<br>2. Carga un archivo CSV (documento, nombre, grado, correo de contacto) o rellena el formulario individual, donde el correo de contacto es un campo obligatorio.<br>3. El sistema valida que no existan duplicados por documento ni por correo.<br>4. Persiste los datos, genera una contraseña aleatoria por cada elector y encola su envío por correo (RN-9). |
