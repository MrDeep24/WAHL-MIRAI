...
| **Descripción** | Permite al Administrador revisar cada postulación pendiente, verificar sus propuestas, plan de gobierno y documentos de requisito, y decidir si la aprueba (con o sin excepción), o la rechaza indicando obligatoriamente el motivo y si dicho rechazo es definitivo o si permite al candidato volver a inscribirse editando sus requisitos. |
| **Prioridad** | Alta |
| **Precondición** | Sesión de Administrador activa; existe al menos una candidatura en estado 'Pendiente'. |
| **Postcondición** | La candidatura pasa a estado 'Aprobado' (visible en el tarjetón) o 'Rechazado' (no visible); el elector recibe una notificación por correo con el resultado y la indicación de si puede subsanar. |
